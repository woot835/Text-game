...
public class Start {
		...
		while (runs) {
			...
			while (enemyHealth > 0) {
				...
				if (answer.equals("FIGHT")) { //Lets heroes fight.
					...
					if (target == 0 && player.Hero1Health > 0) { // Checks if hero is chosen by RNG and is alive
					...
					if (player.Hero1Health > 0) { // Yells who did how much damage , or if they are dead
				...
				} else if (answer.equals("HEAL")) { // Lets the party heal if they have healing potions.
					if (player.numHP > 0) {
				...
				} else if (answer.equals("PARTY")) { // Shows how much health the party has.
					...
					int Encounterrate = player.RandomEncounter - 1;
					if (Encounterrate < 0) {
						System.out.println( // Happens if you RUN too many times, ends the program
								"The Party of " + Partyname + " is too frightened by the dungeon and have fled.");
						System.out.println("You lose \n");
						return;
					}
				} else { // If given command is incorrect, asks for the person to retry.
	...
	public static boolean areAllPlayersDead(Player player) { // to check if all players are dead
